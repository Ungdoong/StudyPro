  ...
  <v-card class="pa-0">
    <v-row no-gutters class="pa-0" border="0px">
      <v-col cols="12" md="6" class="d-none d-md-block">
        <v-card height="130" outlined tile flex id="remote_block_0">
          <video
            ...
            width="100%"
            height="100%"
            @contextmenu="showProfileMenu($event, 0)"
          ></video>
          <img
            class="mute"
            @click="mute($event)"
      ...
      <v-col
        ...
        @contextmenu="showProfileMenu($event, i)"
      >
        <v-card
          height="130"
          class="ma-0 pa-0"
...
export default {
  ...
  methods: {
    ...
    redrawing(e) {
      ...
      if (i == 0) {
        ...
        const delete_stream = this.video_streamings[i]
          ? this.local_stream
          : this.local_dummy_stream;
        const add_stream = this.video_streamings[i]
          ? this.local_dummy_stream
          : this.local_stream;
        this.video_streamings[i] = !this.video_streamings[i];
        temp_btn.src = this.video_streamings[i]
          ? this.camera_on_img
          : this.camera_off_img;
        ...
        const temp_stream = this.video_streamings[i]
          ? this.local_dummy_stream
          : this.remote_streams[i];
    ...
    async getPeerConnection(user_id) {
      ...
      t_pc.onaddstream = (event) => {
        ...
        while (remote_block.firstChild)
          remote_block.removeChild(remote_block.lastChild);
    ...
    deleteBorder(video_num, before_id) {
      ...
      this.remote_videos[video_num].removeChild(
        this.remote_videos[video_num].lastChild
      );
    },
  ...
  mounted() {
    ...
    this.socket.on("leave", (message) => {
      const video_num =
        this.connected_users.indexOf(message.user_id) === -1
          ? this.connected_users.indexOf(`${message.user_id}`)
          : this.connected_users.indexOf(message.user_id);
      ...
      while (temp_video.firstChild)
        temp_video.removeChild(temp_video.lastChild);
    ...
    this.socket.on("message", (data) => {
      if (data.message.type === "offer") {
        ...
        this.user_profiles[this.connected_users.indexOf(data.from)] =
          data.from_profile;
